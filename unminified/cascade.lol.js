(async () => {
	if (console.log("%c cascade.rip [log] %c\n	cascade.rip loaded", "color: #f0f0f0ff; font-size: 3rem", "color: #8000ff; font-size: 1rem"), console.log("%c	gui.js", "color: #ffffffff; font-size: 1rem"), console.log("%c	cascade.lol is on top!%c  https://conspiracy.rip", "color: #ffd000; font-size: 1rem", ""), document.querySelector("script[src*='bfs/index.js']") && !window.clearId) {
  ...
  --contentBackground: #292929;
		...
		innerHTML: '<span id="typewriter-text" style="text-shadow: 1px 1px rgb(0 0 0 / 40%)"></span>',
		onclick: () => window.open("https://conspiracy.rip", "_blank").focus()
	}), $ = l("div", {
		id: "controlButtons",
		style: {
			display: "flex",
		...
		onclick: function() {
			let e = !1;
			return () => {
				for (var t of [...p.children]) t != $ && (e ? t.style.display = t.style._display : (t.style._display = t.style.display, t.style.display = "none"));
				p.style.height = e ? "100%" : "55px", p.style.width = e ? "100%" : "165px", _.style.top = parseInt(_.style.top) + (_.offsetHeight - 55) * (e ? -1 : 1) + "px", _.style.left = parseInt(_.style.left) + (_.offsetWidth - 165) * (e ? -1 : 1) + "px", _.style.pointerEvents = e ? "unset" : "none", e = !e
			}
		}()
	}), l("button", {
		style: {
		...
		innerHTML: '<span style="text-shadow: 1px 1px rgb(0 0 0 / 40%)">cascade</span>'
	}, l("a", {
		className: "bigButton",
		style: {
			cursor: "pointer",
		...
		target: "_blank",
		href: "https://discord.gg/vixencommunity",
	...
	async function k(e, t, a) {
		b.innerHTML = "", v.firstChild.innerText = e + (a ? "" : " cascade.lol"), b.append(v);
			...
			if (!u) {
				...
				if (p.dataset.description = i, p.onclick = (function({
					...
					for (let h = 0; h < s.length; h++) {
						...
						} else(g = document.createElement("input")).classList.add("cheatInput"), "number" == $ && (g.type = "number", g.min = y, g.max = _, g.value = f || (null != y ? y : 0)), g.placeholder = m, g.style.textAlign = "center", g.onkeyup = p.onclick, p.appendChild(g)
					}
				t[o].element = p
			}
	...
	let C = {
		global: [{
			...
			run: async function(gameId, e, t, a, o) {
				...
				var stateNode = i();
				if (!stateNode?.props?.liveGameController?._liveApp?.firebase && !window.location.href.includes("blooket.com/play/lobby")) {
					alert("You must be in a game to use the flooder!");
					return;
				}
				if (!stateNode?.props?.liveGameController?._liveApp?.firebase && !window.location.href.includes("blooket.com/play/")) {
					alert("You must be in a game to use the flooder!");
					return;
				}
				var s = stateNode?.props?.liveGameController?._liveApp?.firebase;
				var useFirebase = !!s;
				async function l(e, t) {
					try {
						...
						if (r.success && useFirebase) {
							...
							await i.database().ref(`${gameId}/c/${t}`).set({
								b: a,
								bg: o
							});
							C.alerts[0].addLog(`Bot ${t} joined successfully!`);
						} else if (r.success) {
							// Fallback: Try direct API call
							await fetch(`https://blooket.com/api/firebase/setValue`, {
								body: JSON.stringify({
									path: `${gameId}/c/${t}`,
									value: { b: a, bg: o }
								}),
							...
							C.alerts[0].addLog(`Bot ${t} joined (fallback method)!`);
						} else {
							alert(`Connect error: ${r.msg || 'Unknown error'}`);
						}
					} catch (error) {
						C.alerts[0].addLog(`Error joining bot ${t}: ${error.message}`);
					}
				}
				async function c() {
					var a, o = gameId,
			...
			run: async function() {
				...
				if (document.body.append(e), window.alert = e.contentWindow.alert.bind(window), e.remove(), location.href.includes("play.blooket.com")) {
					...
					let stateNode, userData;
					try {
						stateNode = function e(t = document.querySelector("body>div")) {
							let node = Object.values(t)[1]?.children?.[0]?._owner?.stateNode;
							return node ? node : e(t.querySelector(":scope>div"))
						}();
						
						console.log("StateNode found:", !!stateNode);
						console.log("StateNode props:", stateNode?.props);
						
						userData = stateNode?.props?.user?.data;
						
						if (!userData) {
							// Try alternative paths
							userData = stateNode?.props?.client?.user?.data || 
									  stateNode?.state?.user?.data ||
									  stateNode?.user?.data;
						}
						...
						if (!userData) {
							alert("Could not access user data. Please make sure you're logged in and on play.blooket.com");
							return;
						}
			...
			run: function() {
				...
				n.type = "text", n.className = "chat-input", n.placeholder = "Type a message...", o.appendChild(n), o.style.position = "fixed", o.style.bottom = "20px", o.style.right = "20px", o.style.width = "300px", o.style.backgroundColor = "#fff", o.style.border = "1px solid #ccc", o.style.boxShadow = "0px 0px 10px rgba(0, 0, 0, 0.2)", r.addEventListener("click", () => {
					...
					13 === a.keyCode && (function a(o) {
						var r, n, l, c, d, u, p, h = function e(t) {
						...
						if (h) switch (h.cmd) {
							...
							case "dump":
								...
								break;
							case "unlock":
								u = h.args.join(" "), (p = webpackJsonp.push([
									[], {
										1234(e, t, a) {
											t.webpack = a
										}
								...
								]).webpack("MDrD").a)[u = Object.keys(p).find(e => u.toLocaleLowerCase() === e.toLocaleLowerCase())] ? (e().stateNode.state.unlocks.push(u), e().stateNode.forceUpdate()) : s("No blook with that name was found!");
								break;
							case "code":
						...
						update(t, a) {
							...
							if (i.rarity && 0 !== this.numExplosions && (this.nextParticle -= a, this.nextParticle <= 0)) {
								switch (i.rarity) {
									...
									case "Epic":
										for (var d = 0; d < 2; d++) {
											var u = i.particles.get();
											u && u.spawn.apply(u, Object.values(r(d % 2 == 0 ? "left-shower" : "right-shower")).concat("epic-" + Object(o.m)(1, 8)))
										}
										break;
									case "Legendary":
										for (var p = 0; p < 3; p++) {
			...
			run: function(e) {
				...
				// Check if user selected Hitler and convert to special code
				if (e === "Hitler") {
					e = "1#0#1#0#1$3#0#0#1#6#0#0$0";
				}
		...
		crypto: [{
			...
			name: "Send Ad Text",
			description: "Sends custom text that overrides your blook",
			inputs: [{
				name: "Player",
				type: "options",
				options() {
					...
					try {
						let stateNode = Object.values(document.querySelector("body div[id] > div > div"))[1].children[0]._owner.stateNode;
						return new Promise(resolve => {
							if (stateNode.props.liveGameController._liveApp) {
				...
				name: "Text",
				type: "text",
				placeholder: "Enter ad text (no colons)"
			}],
			run: function(player, text) {
				...
				try {
					...
					// Set the blook to Parrot with the ad text
					let adText = "Parrot:" + text.repeat(500);
					stateNode.props.liveGameController.setVal({
						path: `c/${stateNode.props.client.name}/b`,
						val: adText
					});
					
					// Set the tat to target player with 100 value
					if (player) {
						stateNode.props.liveGameController.setVal({
					...
					alert("Ad text sent successfully!");
				} catch (error) {
					console.error("Error sending ad text:", error);
					alert("Failed to send ad text. Make sure you're in a game.");
				}
			}
		}],
		defense: [{
				...
				let e = ["⁰", "\xb9", "\xb2", "\xb3", "⁴", "⁵", "⁶", "⁷", "⁸", "⁹"],
					...
					a = e => {
						...
						if (e >= 1e3) {
							...
							if (r < o.length) {
								let i = "";
								for (let n = 3; n >= 1 && !((i = parseFloat((0 !== r ? e / 1e3 ** r : e).toPrecision(n)).toString()).replace(/[^a-zA-Z 0-9]+/g, "").length <= 3); n--);
								Number(i) % 1 != 0 && (i = Number(i).toFixed(1)), a = i + o[r]
							} else {
								let s = e,
									l = 0;
								for (; s >= 100;) s = Math.floor(s / 10), l += 1;
								a = `${s/10} \xd7 10${t(l+1)}`
							}
						}
						return a
		...
		factory: [{
			...
			inputs: [{
				...
				options: [{
					...
					name: "1#0#1#0#1$3#0#0#1#6#0#0$0 (Heetler)",
					color: "#ff0000",
					class: "",
					rarity: "Mystical",
					cash: [999999, 999999, 999999, 999999, 999999],
					time: [0, 0, 0, 0, 0],
					price: [0, 0, 0, 0]
				}]
			}].map(e => ({
		...
		cafe: [{
			name: "Infinite Food",
			description: "Gives you infinite food",
			run: function() {
				try {
					let stateNode = this.findStateNode();
					if (stateNode) {
						stateNode.setState({
							foods: Array(5).fill({
								name: "Pizza",
						...
						console.log("Infinite Food activated");
					} else {
						console.log("Could not find game state");
					}
				} catch (e) {
					console.error("Infinite Food error:", e);
				}
			},
			findStateNode: function() {
				// Simple state node finder
				try {
					// Method 1: Standard Blooket selector
					let node = Object.values(document.querySelector("body div[id] > div > div"))[1]?.children[0]?._owner?.stateNode;
					if (node && node.setState) return node;
					
					// Method 2: Alternative selector
					node = Object.values(document.querySelector("#app > div > div"))[1]?.children[0]?._owner?.stateNode;
					if (node && node.setState) return node;
					
					// Method 3: Search all React components
					...
					console.error("State node finder error:", e);
					return null;
				}
			}
		}, {
			name: "Max Customers",
			description: "Sets all customers to max satisfaction",
			run: function() {
				try {
					let stateNode = this.findStateNode();
					if (stateNode && stateNode.state && stateNode.state.customers) {
						stateNode.state.customers.forEach(customer => {
							customer.satisfaction = 100;
							customer.tip = 100;
						});
						console.log("Max Customers activated");
					} else {
						console.log("Could not find customers or game state");
					}
				} catch (e) {
					console.error("Max Customers error:", e);
				}
			},
			findStateNode: function() {
				try {
			...
			name: "Set Money",
			description: "Sets your money to specified amount",
			inputs: [{
				name: "Amount",
				type: "number"
			}],
			run: function(amount) {
				try {
					...
					if (stateNode && stateNode.setState) {
						// Try different possible money property names
						let moneyProps = ['money', 'cash', 'coins', 'gold'];
						let found = false;
						
						for (let prop of moneyProps) {
							if (stateNode.state && stateNode.state[prop] !== undefined) {
								stateNode.setState({
									[prop]: amount
								});
								console.log(`Set ${prop} to`, amount);
								found = true;
								break;
							}
						}
						...
						if (!found) {
							// Try setting all possible money properties
							stateNode.setState({
								money: amount,
								cash: amount,
								coins: amount,
							...
							console.log("Set all money properties to", amount);
						}
					} else {
						console.log("Could not find game state");
					}
				} catch (e) {
					console.error("Set Money error:", e);
				}
			},
			findStateNode: function() {
				try {
			...
			name: "Auto Serve",
			description: "Automatically serves customers",
			type: "toggle",
			enabled: !1,
			data: null,
			run: function() {
				this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null, console.log("Auto Serve disabled")) : (this.enabled = !0, this.data = setInterval(() => {
					try {
						let stateNode = this.findStateNode();
						if (stateNode && stateNode.state && stateNode.state.customers) {
							stateNode.state.customers.forEach(customer => {
								if (customer.ready && !customer.served && stateNode.serveCustomer) {
									stateNode.serveCustomer(customer);
								}
							});
						}
					} catch (e) {
						console.error("Auto serve error:", e);
					}
				}, 100), console.log("Auto Serve enabled"))
			},
			findStateNode: function() {
				try {
					let node = Object.values(document.querySelector("body div[id] > div > div"))[1]?.children[0]?._owner?.stateNode;
			...
			name: "Instant Cook",
			description: "Makes all food cook instantly",
			type: "toggle",
			enabled: !1,
			data: null,
			run: function() {
				this.enabled ? (this.enabled = !1, clearInterval(this.data), this.data = null, console.log("Instant Cook disabled")) : (this.enabled = !0, this.data = setInterval(() => {
					try {
						let stateNode = this.findStateNode();
						if (stateNode && stateNode.state && stateNode.state.foods) {
							stateNode.state.foods.forEach(food => {
								if (food.cooking) {
									food.time = 0;
									food.cooking = false;
								}
						...
						console.error("Instant cook error:", e);
					}
				}, 50), console.log("Instant Cook enabled"))
			},
			findStateNode: function() {
				try {
					let node = Object.values(document.querySelector("body div[id] > div > div"))[1]?.children[0]?._owner?.stateNode;
			...
			name: "Upgrade All",
			description: "Upgrades all equipment to max level",
			run: function() {
				try {
					let stateNode = this.findStateNode();
					if (stateNode && stateNode.setState) {
						...
						console.log("Upgrade All activated");
					} else {
						console.log("Could not find game state");
					}
				} catch (e) {
					console.error("Upgrade All error:", e);
				}
			},
			findStateNode: function() {
				try {
		...
		gold: [{
			name: "Display Text",
			description: "Displays custom text on the screen",
			inputs: [{
				name: "Text",
				type: "text",
				placeholder: "Enter the text you want to display"
			...
			run: function(e) {
				...
				// Try to find and modify the gold text element
				let goldElement = document.querySelector('._totalGoldText_1yzop_188');
				if (goldElement) {
					goldElement.innerText = e;
				} else {
					// Fallback: try other possible selectors
					let elements = document.querySelectorAll('[class*="totalGold"], [class*="goldText"], div[style*="white-space: nowrap"]');
					elements.forEach(el => {
						if (el.innerText === "0" || el.style.whiteSpace === "nowrap") {
							el.innerText = e;
						}
					});
				}
				
				// Also try the tat method as backup
				a.props.liveGameController.setVal({
					path: `c/${a.props.client.name}`,
					val: {
						b: a.props.client.blook,
				...
				e.enabled ? (clearInterval(e.data), e.data = null, e.enabled = !1) : (e.enabled = !0, e.data = setInterval(() => {
					if ("gold" === ("gold" === window.location.pathname.split("/")[2] && "gold") && e.enabled) try {
						if ("prize" === t().stateNode.state.stage) {
							let {
								choices: a
							} = t().stateNode.state, o = document.querySelector("div[class*='regularBody']").children[1];
							o && (document.querySelectorAll(".chest-esp").length ? a.forEach((e, t) => {
								3 == o.children.length && o.children[t].children[1].innerText != e.text && (o.children[t].children[1].innerText = e.text)
							}) : a.forEach((e, t) => {
								...
								try {
									o.children[t].appendChild(textElement)
								} catch (a) {
									console.log(a)
								}
							}))
				...
				e.enabled || t.enabled ? (clearInterval(e.data), clearInterval(t.data), e.data = null, t.data = null, e.enabled = !1, t.enabled = !1) : (e.enabled = !0, t.enabled = !0, e.data = setInterval(() => {
					...
					if ("gold" === ("gold" === window.location.pathname.split("/")[2] && "gold") && t.enabled) try {
						if ("prize" === a().stateNode.state.stage) {
							let {
								choices: e
							} = a().stateNode.state, o = document.querySelector("div[class*='regularBody']").children[1];
							o && (document.querySelectorAll(".chest-esp").length ? e.forEach((e, t) => {
								3 == o.children.length && o.children[t].children[1].innerText != e.text && (o.children[t].children[1].innerText = e.text)
							}) : e.forEach((e, t) => {
		...
		extras: [{
			...
			name: "Tab Cloaker",
			description: "Changes the tab image and name with pre-selected options",
			inputs: [{
				name: "Preset",
				type: "options",
				options: [{
					...
					value: {
						title: "Yahoo!",
						icon: "https://cdn-icons-png.flaticon.com/512/3128/3128296.png"
					}
				}, {
					name: "Discord",
					value: {
					...
					value: {
						title: "YouTube",
						icon: "https://favicon.im/youtube.com"
					}
				}, {
					name: "Twitter",
					value: {
						title: "Twitter",
						icon: "https://favicon.im/x.com"
					}
				}, {
					name: "Instagram",
					value: {
						title: "Instagram",
						icon: "https://favicon.im/instagram.com"
					}
				}, {
					name: "Reddit",
					value: {
						title: "Reddit",
						icon: "https://favicon.im/reddit.com"
					}
				}, {
					name: "Custom",
					value: null
				...
				type: "text",
				placeholder: "Enter custom icon URL (if Custom selected)"
			}, {
				name: "Custom Title",
				type: "text",
				placeholder: "Enter custom title (if Custom selected)"
			}],
			run: function(e, t, a) {
				var preset;
				try {
					// Try to parse as JSON string first
					preset = typeof e === 'string' ? JSON.parse(e) : e;
				} catch (error) {
					preset = null;
				}
				...
				if (preset && preset.title && preset.icon) {
					// Use preset values
					iconUrl = preset.icon;
					title = preset.title;
				} else {
					// Use custom values
					iconUrl = t || "https://www.blooket.com/favicon.ico";
					title = a || "Blooket";
				}
		...
		settings: [{
			...
			run: function(e) {
				...
				for (let a in e = {
						...
						...JSON.parse(e)
					}, c.setItem("theme", e), e) u.sheet.cssRules[0].style.setProperty("--" + a, e[a])
			}
		}, {
			...
			inputs: [{
				...
				options: [{
					...
					value: {
						...
						cheatList: "#2d3748",
						infoColor: "#4a5568",
						defaultButton: "#4299e1",
						inputColor: "#2d3748",
						enabledButton: "#48bb78",
						textColor: "#ffffff",
						disabledButton: "#718096",
			...
			run: function(e) {
				for (let t in c.setItem("theme", {
						...c.data.theme,
						...e
					}), e) u.sheet.cssRules[0].style.setProperty("--" + t, e[t])
			}
		}, {
			name: "Scale",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--backgroundColor", e), c.setItem("theme.backgroundColor", e)
			}
		}, {
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--cheatList", e), c.setItem("theme.cheatList", e)
			}
		}, {
			name: "Info Color",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--infoColor", e), c.setItem("theme.infoColor", e)
			}
		}, {
			name: "Button Color",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--defaultButton", e), c.setItem("theme.defaultButton", e)
			}
		}, {
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--disabledButton", e), c.setItem("theme.disabledButton", e)
			}
		}, {
			name: "Text Color",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--textColor", e), c.setItem("theme.textColor", e)
			}
		}, {
			name: "Input Color",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--inputColor", e), c.setItem("theme.inputColor", e)
			}
		}, {
			name: "Content Color",
			...
			run: function(e) {
				u.sheet.cssRules[0].style.setProperty("--contentBackground", e), c.setItem("theme.contentBackground", e)
			}
		}],
		alerts: [{
			element: l("div", {
				...
				innerText: "thank you for using cascade, this blocks blooket's anticheat from banning you. have fun!"
			})))),
			addLog(e, t) {
				...
				try {
					...
					this.connection.on("value", e => {
						...
						if (r && this.diffObjects(this.data, r)) {
							var i, n, s, l, c, d, u, p, h = this.diffObjects(this.data, r);
							this.data = r;
							let m;
							switch (a) {
								case "racing":
								...
								case "dino":
									...
									break;
								case "cafe":
									for (let v in h) h[v].up && ([u, p] = h[v].up.split(":"), p) && this.addAlert(v, t[r[v].b]?.url, `upgraded ${u} to level ` + p);
									m = Object.entries(r).map(([e, {
										b: t,
										ca: a
									}]) => ({
	...
	if (_.addEventListener("mousemove", e => {
		...
		window.fetch.call = function() {
			...
			C.alerts?.[0].addLog("cascade blocked anticheat :3")
		}
	} else console.log("already run")

	const messages = [
		"epstein was a good man.",
		"Hyperbaiter.",
		"Its coming out!",
		"Hitler, was a amazing dude!",
		"i shit out my mouth, piss out my ass.",
		"indian walmart shit",
		"12 inch dildo",
		"fuck the jews",
		"diddy heil epstein!",
		"all from diddys party",
		"diddy heil epstein!"
	];
